...
<form name="course-details" name="details" method="post" action="<?= $controller->link_for('my_courses/set', $id) ?>" <?= $dialog_attr ?> class="default collapsable">
...
<? else: ?>
    <? foreach ($attributes as $attribute): ?>
        <label>
            ...
            <?= $this->render_partial("my_courses/_input", ['input' => $attribute]) ?>
        </label>
    <? endforeach; ?>
<? endif; ?>
        ...
        <legend><?= _('zugehörigkeit zu folgende Module') ?></legend>
    ...
    <? foreach ($institutional as $inst): ?>
        ...
        <? if ($inst['type'] === 'select' && !$inst['choices'][$inst['value']]): ?>
            <? $name = get_object_name($inst['value'], 'inst'); ?>
             <?= htmlReady($name['name']) ?>
        <? else: ?>
            <?= $this->render_partial('my_courses/_input', ['input' => $inst]) ?>
        <? endif; ?>
        </label>
    <? endforeach; ?>
<? endif; ?>
...
<!--    <fieldset <?= !isset($flash['open']) || $flash['open'] != 'bd_personal' ? 'class="collapsed"' : ''?>>
            ...
            <? if ($perm_dozent && !$dozent_is_locked): ?>
                <span class="actions">
                    <?= MultiPersonSearch::get('add_member_dozent' . $course_id)
                            ...
                            ->setExecuteURL($controller->url_for('my_courses/add_member/' . $course_id))
        ...
        <? else: ?>
            <? foreach (array_values($dozenten) as $num => $dozent) : ?>
                ...
                <? if ($perm_dozent && !$dozent_is_locked): ?>
                    <? if ($num > 0) : ?>
                        <a href="<?= $controller->link_for('my_courses/priorityupfor', $course_id, $dozent['user_id'], 'dozent') ?>" <?= $dialog_attr ?>>
                            <?= Icon::create('arr_2up', Icon::ROLE_SORT)->asImg(['class' => 'middle']) ?>
                        </a>
                    <? endif; ?>
                    <? if ($num < count($dozenten) - 1): ?>
                        <a href="<?= $controller->link_for('my_courses/prioritydownfor', $course_id, $dozent['user_id'], 'dozent') ?>" <?= $dialog_attr ?>>
            ...
            <? if ($perm_dozent && !$dozent_is_locked) : ?>
                <span class="actions">
                    <?= MultiPersonSearch::get('add_member_deputy' . $course_id)
                            ...
                            ->setDataDialogStatus(Request::isXhr())
                            ->setJSFunctionOnSubmit(Request::isXhr() ? 'jQuery(this).closest(".ui-dialog-content").dialog("close");' : false)
                            ->setExecuteURL($controller->url_for('my_courses/add_member/' . $course_id . '/deputy'))
                            ->render() ?>
                </span>
            <? endif; ?>
            </caption>
        ...
        <? else: ?>
            <? foreach ($deputies as $deputy) : ?>
                <tr>
                    ...
                    <? if ($perm_dozent && !$dozent_is_locked): ?>
                        <?= Icon::create('trash')->asInput([
                            'formaction'   => $controller->url_for('my_courses/deletedeputy', $course_id, $deputy['user_id']),
                            'data-confirm' => _('Soll die Person wirklich entfernt werden?'),
                        ]) ?>
                    <? endif; ?>
                    </td>
    ...
    <? endif ?>
        ...
        <table class="default">
            ...
            <? if ($perm_dozent && !$tutor_is_locked): ?>
                ...
                <?= MultiPersonSearch::get('add_member_tutor' . $course_id)
                        ...
                        ->setDataDialogStatus(Request::isXhr())
                        ->setJSFunctionOnSubmit(Request::isXhr() ? 'jQuery(this).closest(".ui-dialog-content").dialog("close");' : false)
                        ->setExecuteURL($controller->url_for('my_courses/add_member/' . $course_id . '/tutor'))
                        ->addQuickfilter(sprintf(_('%s der Einrichtung'), get_title_for_status('dozent', 2)), $lecturersOfInstitute)
        ...
        <? else: ?>
            <? foreach (array_values($tutoren) as $num => $tutor): ?>
                ...
                <? if ($perm_dozent && !$tutor_is_locked): ?>
                    <? if ($num > 0) : ?>
                        <a href="<?= $controller->link_for('my_courses/priorityupfor', $course_id, $tutor['user_id'], 'tutor') ?>" <?= $dialog_attr ?>>
                            <?= Icon::create('arr_2up', Icon::ROLE_SORT)->asImg(['class' => 'middle']) ?>
                        </a>
                    <? endif; ?>
                    <? if ($num < count($tutoren) - 1) : ?>
                        <a href="<?= $controller->link_for('my_courses/prioritydownfor', $course_id, $tutor['user_id'], 'tutor') ?>" <?= $dialog_attr ?>>
                            <?= Icon::create('arr_2down', Icon::ROLE_SORT)->asImg(['class' => 'middle']) ?>
                        </a>
                    <? endif; ?>
                        <?= Icon::create('trash')->asInput([
...
<? else: ?>
    <? foreach ($descriptions as $description): ?>
        <? if ($description['type'] == 'datafield'): ?>
            <?= $this->render_partial('my_courses/_input', ['input' => $description]) ?>
        <? else : ?>
        <label>
