...
class WizardAController extends AuthenticatedController
{
    /**
    ...
    public function before_filter (&$action, &$args)
    {
        ...
        $this->steps = array(
            array(
                "name" => "Studiendaten",
                "classname" => "BasicDataWizarStep"
            ),
array(
    "name" => "Studiendaten",
    "classname" => "LVGroupsWizarStep"
)
...
// We are after the last step -> all done, show summary.
            if ($next_step >= sizeof($this->steps)) {
                $this->redirect($this->url_for('wizardA/summary', $next_step, $temp_id));
// Redirect to next step.
            } else {
                $this->redirect($this->url_for('wizardA/step', $next_step, $this->temp_id));
            }
        }
    }
    ...
    public function summary_action($stepnumber, $temp_id)
    {
        ...
        if (isset($_SESSION['coursewizard'][$this->temp_id]['source_id'])) {
            $course = Veranstaltungsanmelden::find( $_SESSION['coursewizard'][$this->temp_id]['source_id']);

            $course->astatus='fertig';
            $course->status='fertig';


            $this->source_course = Course::find($_SESSION['coursewizard'][$this->temp_id]['source_id']);
            if($this->source_course == Course::find($_SESSION['coursewizard'][$this->temp_id]['source_id'])){
                $course->store();
    ...
    public function forward_action($step_number, $temp_id)
    {
        ...
        $this->redirect($this->url_for('wizardA/step', $step_number, $this->temp_id));
    }
    ...
    public function copy_action($id) {
      ...
      $this->redirect($this->url_for('wizardA/step/0/' . $this->temp_id, ['cid' => '']));
